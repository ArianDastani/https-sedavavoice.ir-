...
(function ($) {
  ...
  const type_text = function () {
    var typed = new Typed(".auto-type", {
      strings: ["طراح UX/UI","طراح سایت", "گوینده", "فریلنسر","تبلیغات"],
      typeSpeed: 65,
      backSpeed: 70,
      backDelay: 900,
      loop: true,
...
------------------------------------------------------------------------------------- */
  ...
  const counter = function () {
    if ($(document.body).find(".wedo-section").hasClass("counter-scroll")) {
      $(window).on("load scroll", function () {
        let oTop = $(".wrap-counter").offset().top - window.innerHeight;
...
const number = function () {
  $(".number").each(function () {
    var size = $(this).text().split(".")[1]
      ...
      : 0;
    var startVal = parseFloat($(this).text());
    var endVal = parseFloat($(this).text());
    if (isNaN(startVal)) startVal = 0;
    if (isNaN(endVal)) {
       endVal = $(this).text();
    } else {
       endVal = parseFloat($(this).text());
    }
    $(this)
      .prop("Counter", startVal)
      .animate(
        {
          Counter: endVal,
        },
        {
          ...
          step: function (now) {
            ...
            if (size > 0) {
              now = now.toFixed(size);
            } 
            $(this).text(parseFloat(now).toString());
          }
        }
      );
  });
...
let observer = new IntersectionObserver((entries) => {
  entries.forEach((entry) => {
    ...
    if(entry.isIntersecting) {
       const stickyTarget = entry.target;
       const targetTop = stickyTarget.getBoundingClientRect().top;

       if(targetTop <= 0) {
          document.querySelector('.wedo-section').classList.add('sticky-top');
          stickyTarget.classList.remove('sticky-bottom');
       } else {
          document.querySelector('.sticky-sidebar').classList.remove('sticky-top');
          stickyTarget.classList.add('sticky-bottom');
       }
    }
